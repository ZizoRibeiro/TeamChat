...
RSpec.describe ChannelsController, type: :controller do
  ...
  describe 'GET #show' do
    ...
    context 'User is team member' do
      ...
      it 'retuns right channel values' do
        response_hash = JSON.parse(response.body)

        expect(response_hash["slug"]).to eql(@channel.slug)
        expect(response_hash["user_id"]).to eql(@channel.user_id)
        expect(response_hash["team_id"]).to eql(@channel.team_id)
      end
  ...
  describe 'DELETE #destroy' do
    context 'User is a team member' do
      ...
      context 'User isnt the team or channel owner' do
        it 'returns http forbidden' do
          team = create(:team)
          team.users << @current_user
          @channel = create(:channel, team: team)
